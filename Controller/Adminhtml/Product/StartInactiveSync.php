...
/**
 * Class StartInactiveSync
 * @package Ced\Etsy\Controller\Adminhtml\Product
 */
class StartInactiveSync extends Action
{
    /**
    ...
    public function execute()
    {
        ...
        try {
            $results = $this->helper->ApiObject()->findAllShopListingsInactive(
                [
                    'params' => [
                        'shop_id' => $shopName,
                        'limit' => 100,
                ...
                try {
                    if (isset($item['sku']) && !empty($item['sku'])) {
                        foreach ($item['sku'] as $sku) {
                            ...
                            if ($product) {
                                if ($product->getEtsyCode() != $item['state']) {
                                    $product->setEtsyCode($item['state']);
                                    $product->setEtsyListingId($item['listing_id']);
                                    $product->getResource()->save($product);
                                    $skus[] = $product->getSku().": ".$product->getEtsyCode();
                                } else {
                                    $skus[] = $product->getSku().": ".$product->getEtsyCode();
                                }                                    
                            } else {
