 ...
 */
class Massimgsync extends Action
{
    ...
    {
        $pids = $this->filter->getCollection($this->catalogCollection->create()->addFieldToFilter('etsy_product_status', 'uploaded'))->getAllIds();
        $ids = array_chunk($pids, 10);
        if (!empty($ids)) {
            ...
            $this->messageManager->addErrorMessage(__('No product available for Image sync.'));
