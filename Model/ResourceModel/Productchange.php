...
class Productchange extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
    ...
    public function _construct()
    {
        ...
        $this->_init('etsy_product_change', 'id');
    ...
    public function deleteFromProductChange($productIds, $type)
    {
        ...
        $condition = "{$this->getTable('etsy_product_change')}.product_id in (" . $dbh->quote($productIds).")";
        //. " AND {$this->getTable('jet/profileproducts')}.profile_id = " . $dbh->quote($profileId);
        $condition .= " AND {$this->getTable('etsy_product_change')}.cron_type = '".$type."'";
        $dbh->delete($this->getTable('etsy_product_change'), $condition);
        return $this;
    }
